          ...
          for($ki=0;$ki<=8;$ki++)
          {
              for($kj=$ki+1;$kj<=9;$kj++)
              {
                 if($from==$row[$ki] && $to==$row[$kj])
                {
                    ...
                    $cost=(int)$row1['rate'];
                    $ncost=(int)(($cost/6)*($kj-$ki));
                    echo "<tr>";
                    echo "<td style='width: 100px;'><input type='hidden' name='trainid".htmlspecialchars($n)."' value=".$row1["trainid"].">". $row1["trainid"] . "</td>";
                    echo "<td style='width: 150px;'><input type='hidden' name='tname".htmlspecialchars($n)."' value=".$row1["tname"].">" . $row1["tname"] . "</td>";
                    echo "<td style='width: 150px;'><input type='hidden' name='fromcity".htmlspecialchars($n)."' value=".$from." >" .$from. "</td>";
                    echo "<td style='width: 150px;'><input type='hidden' name='tocity".htmlspecialchars($n)."' value=".$to.">" . $to. "</td>";
                    echo "<td style='width: 150px;'><input type='hidden' name='dt".htmlspecialchars($n)."' value=".$row1["dateofjourney"].">" . date("d-m-Y",strtotime($row1['dateofjourney'])) . "</td>";
                    if($row1['nooftickets']>=0)
                    {
                    echo "<td style='width: 100px;'><input type='hidden' name='numb".htmlspecialchars($n)."' value=".$row1["nooftickets"].">" . $row1["nooftickets"] . "</td>";
                     }
                    else{
                    echo "<td style='width: 100px;'><input type='hidden' name='numb".htmlspecialchars($n)."' value=".$row1["nooftickets"].">WL" . $row1["nooftickets"] . "</td>";
                     }
                    echo "<td style='width: 100px;'><input type='hidden' name='rate".htmlspecialchars($n)."' value=".$ncost.">" .$ncost. "</td>";
                    echo "<td style='width: 100px;'><input type='submit' value='BOOK' name=".htmlspecialchars($n)."></td>";
                     echo "</tr>";
                     $n=$n+1;
            }
